const decompress = input => {
  ...
  while(input.length){
    ...
    if(!input.startsWith('(')){
      ...
      // Increase the length of the decompressed data by the length of the decompressed region multiplied by the number of times to repeat
      length += decompress(input.substr(marker.length, reach)) * times
