...
module.exports = input => input.split("\n")
  ...
  .reduce((newList, guest) => {
    newList[0].relations[guest.name] = guest.relations.Reuben = 0;
    newList.push(guest);
    return newList
  }, [{name: "Reuben", relations: {}}])
  .reduce(permute, [])
  .map(seating => seating
    .reduce((store, seat) => ({
      happiness: store.happiness + seat.relations[store.last] || 0,
      last: seat.name
    }), {
      happiness: 0,
      last: seating.slice(0).pop().name
    }))
