    ...
    .reduce((grid, block) => {
      for (let x = block.start.x; x <= block.end.x; x++) {
        for (let y = block.start.y; y <= block.end.y; y++) {
          ...
          if (block.operation === "turn off" && grid[x][y] > 0) grid[x][y] -= 1
          if (block.operation === "toggle") grid[x][y] += 2
        }
      }
      return grid
    }, [])
    .reduce((collumnBrightness, x) => collumnBrightness + x.reduce((rowBrightness, y) => rowBrightness + y, 0), 0)
